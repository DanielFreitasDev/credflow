                ...
                <div key={log.id} className="flex items-start justify-between gap-4 px-4 py-3">
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {log.entityId && <span className="truncate text-xs text-slate-400">#{log.entityId}</span>}
                    </div>
                    {(log.after as Record<string, unknown>) && (
                      <p className="mt-1 truncate text-xs text-slate-500">{JSON.stringify(log.after)}</p>
                    )}
                  </div>
