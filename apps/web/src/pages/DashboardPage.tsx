  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
        <KpiCard icon={<Wallet className="h-6 w-6 text-brand-600" />} tone="bg-brand-50" label="Carteira (a receber)" value={currency(kpis.portfolioOutstanding)} />
        <KpiCard icon={<TrendingUp className="h-6 w-6 text-emerald-600" />} tone="bg-emerald-50" label="Total emprestado" value={currency(kpis.totalLent)} />
        <KpiCard icon={<TrendingUp className="h-6 w-6 text-sky-600" />} tone="bg-sky-50" label="Total recebido" value={currency(kpis.totalReceived)} />
        <KpiCard icon={<AlertTriangle className="h-6 w-6 text-rose-600" />} tone="bg-rose-50" label="Total em atraso" value={currency(kpis.totalOverdue)} />
        <KpiCard icon={<Percent className="h-6 w-6 text-amber-600" />} tone="bg-amber-50" label="Inadimplência" value={percent(kpis.delinquencyRate)} />
        <KpiCard icon={<Users className="h-6 w-6 text-purple-600" />} tone="bg-purple-50" label="Clientes" value={number(kpis.customers)} />
      </div>

      <div className="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-2 xl:grid-cols-2">
        <KpiCard icon={<FileSignature className="h-6 w-6 text-indigo-600" />} tone="bg-indigo-50" label="Contratos ativos" value={number(kpis.activeContracts)} />
        <KpiCard icon={<FileSignature className="h-6 w-6 text-slate-600" />} tone="bg-slate-100" label="Propostas pendentes" value={number(kpis.proposalsPending)} />
      </div>

      <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="card p-5 lg:col-span-2">
          <h3 className="mb-4 font-semibold text-slate-800">Recebimentos futuros (6 meses)</h3>
        ...
        <div className="card p-5">
          <h3 className="mb-4 font-semibold text-slate-800">Clientes por faixa de risco</h3>
          <ResponsiveContainer width="100%" height={280}>
            <PieChart>
              <Pie
      ...
      <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-2">
        <div className="card p-5">
          <h3 className="mb-3 font-semibold text-slate-800">Contratos por status</h3>
          <BreakdownList items={data.contractsByStatus.map((c) => ({ label: contractStatusLabel[c.status] ?? c.status, count: c.count }))} />
        </div>
        <div className="card p-5">
          <h3 className="mb-3 font-semibold text-slate-800">Propostas por status</h3>
...
function BreakdownList({ items }: { items: { label: string; count: number }[] }) {
  ...
  if (!items.length) return <p className="text-sm text-slate-400">Sem dados.</p>;
  return (
