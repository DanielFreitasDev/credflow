...
- **The API runtime image is slim and npm-free**: a `prod-deps` stage runs `npm ci --omit=dev` (so jest/eslint/ts-* are never shipped) and the base image's bundled global **npm/npx is removed** in the runtime stage. Because of that, `prisma` is a **production** dependency (`tsx` already was), and `docker-entrypoint.sh` puts `node_modules/.bin` on `PATH` and calls `prisma`/`tsx` directly instead of `npx`. If you touch the entrypoint or move deps, keep `prisma` + `tsx` resolvable at runtime (the boot runs migrate → backfill → seed → `node dist/main.js`).
