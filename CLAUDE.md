...
## CI/CD & supply chain
- GitHub Actions in `.github/workflows/`: `ci.yml` (api/web typecheck+lint+build+tests; `api-integration` = real Postgres `migrate deploy` + status + **drift check**; `audit`), `security.yml` (CodeQL + dependency-review + gitleaks), `docker.yml` (hadolint + buildx build + Trivy + GHCR `:edge` push on `main`), `release.yml` (tag `v*` → versioned GHCR images + CycloneDX SBOM + Release), `deploy-staging.yml` / `deploy-production.yml`, `scorecard.yml`. All actions are **pinned by commit SHA**; every workflow sets least-privilege `permissions`, `concurrency`, and `timeout-minutes`. CI carries no secrets and is separate from CD. Dependabot config: `.github/dependabot.yml`.
