...
## Database
- Schema: `apps/api/prisma/schema.prisma` (16 models). Four migrations: `0_init` (base schema), `20260621000000_protect_customer_document` (CPF/CNPJ encryption + blind index), `20260621201957_widen_cet_indexes_protect_payments` (CET widened to `Decimal(12,6)`, payment FKs `RESTRICT`), and `20260621211730_harden_indexes_constraints_dunning` (pg_trgm trigram search indexes, `CollectionCase.dunningStage`, PaymentPromise indexes, and range CHECK constraints — the CHECK constraints are appended raw SQL: Prisma doesn't model them, so they live only in migrations and never cause drift). Money columns are `Decimal(14,2)`, rates `Decimal(9,6)`, CET `Decimal(12,6)`. The `postgresqlExtensions` preview is enabled so Prisma manages the `pg_trgm` extension.
