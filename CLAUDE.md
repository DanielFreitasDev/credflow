...
npx prisma generate      # generate client → src/generated (gitignored); REQUIRED after install/checkout & schema changes, before build/typecheck/test
...
- **Login backdrop**: `components/LoginBackground.tsx` renders an animated "credit flow" field behind the login form — a dependency-free **canvas 2D flow field** (particles advected along a layered-sine vector field, drawn as additive glowing trails) colored along the brand gradient `#255EEB→#16C7E6→#30D17A`, reading as value flowing from request (blue) to approval/money (green). It's DPR-capped (≤2×), particle count scales with viewport area (≤240), pauses on tab hide (`visibilitychange`), and cancels its RAF + removes listeners on unmount. Under `prefers-reduced-motion` it draws a single static frame (no loop). The canvas stays transparent (trails fade via `destination-out`) so the page's diagonal navy gradient shows through as a fallback; `LoginPage.tsx` layers it behind content (`relative` parent, content `z-10`) and lifts the form card with `shadow-2xl`/`ring-white/10`. Visual knobs (density, speed, glow, trail length, current direction) are constants at the top of the component.

## Database
- Schema: `apps/api/prisma/schema.prisma` (16 models). Five migrations: `0_init` (base schema), `20260621000000_protect_customer_document` (CPF/CNPJ encryption + blind index), `20260621201957_widen_cet_indexes_protect_payments` (CET widened to `Decimal(12,6)`, payment FKs `RESTRICT`), `20260621211730_harden_indexes_constraints_dunning` (pg_trgm trigram search indexes, `CollectionCase.dunningStage`, PaymentPromise indexes, and range CHECK constraints — the CHECK constraints are appended raw SQL: Prisma doesn't model them, so they live only in migrations and never cause drift), and `20260621230000_harden_lockout_idempotency_indexes` (account-lockout fields on `User`, unique `Payment.idempotencyKey`, `CollectionCase.daysOverdue` + `Customer.internalScore` indexes, dropped redundant `Installment[status]` index, and contract date-ordering + rate-ceiling CHECKs). Money columns are `Decimal(14,2)`, rates `Decimal(9,6)`, CET `Decimal(12,6)`. The `postgresqlExtensions` preview is enabled so Prisma manages the `pg_trgm` extension. **Prisma 7 layout:** the schema uses the `prisma-client` generator emitting a **CommonJS** client into `src/generated/prisma` (gitignored — import it via relative path, not `@prisma/client`); the datasource URL and `db seed` command live in `prisma.config.ts` (Prisma 7 removed `url` from the schema and the `prisma.seed` key from `package.json`); and the runtime client connects through the **`pg` driver adapter** (`@prisma/adapter-pg`, Rust-engine-free) constructed in `PrismaService` — and likewise in `prisma/seed.ts` + `prisma/backfill-documents.ts`, which each build their own `PrismaPg` adapter. The CI drift check uses `prisma migrate diff --from-config-datasource --to-schema` (v7 dropped `--from-url`).
...
- Web tests use **Vitest + React Testing Library** (`apps/web/src/**/*.test.{ts,tsx}`, run with `npm test`). Test files are excluded from the app `tsconfig.json`, so `npm run build`/`typecheck` stay clean; the Vitest config lives in `vite.config.ts` (`test` block). CI's api unit step runs `npm run test:cov` to enforce coverage thresholds.

## CI/CD & supply chain
- GitHub Actions in `.github/workflows/`: `ci.yml` (api/web typecheck+lint+build+tests; `api-integration` = real Postgres `migrate deploy` + status + **drift check**; `audit`), `security.yml` (CodeQL + dependency-review + gitleaks), `docker.yml` (hadolint + buildx build + Trivy + GHCR `:edge` push on `main`), `release.yml` (**reusable** via `workflow_call` + direct tag `v*` push → versioned GHCR images + CycloneDX SBOM + Release; version source unified via `inputs.tag || github.ref_name`), `auto-release.yml` (push to `main` → next SemVer from **Conventional Commits** since the last tag → create tag → calls `release.yml`; `feat`→minor, `fix`/`perf`→patch, `!`/`BREAKING CHANGE`→major, **no release** when only chore/docs/build/ci/test land — it calls release.yml via `workflow_call` because a `GITHUB_TOKEN`-pushed tag can't trigger `release.yml`'s `push: tags`), `deploy-staging.yml` / `deploy-production.yml`, `scorecard.yml`. All actions are **pinned by commit SHA**; every workflow sets least-privilege `permissions`, `concurrency`, and `timeout-minutes`. CI carries no secrets and is separate from CD. Dependabot config: `.github/dependabot.yml`.
- **Required status checks** to set on `main` (repo settings, not in-repo): `api`, `web`, `api-integration`, `audit`, `codeql`, `dependency-review`, `gitleaks`, `docker`.
- **Deploy workflows are inert until opted in**: each deploy job is guarded by `if: vars.DEPLOY_{STAGING,PRODUCTION}_ENABLED == 'true'`, with the rollout step left as a template (no cloud/IaC in the repo). Production uses the `production` GitHub Environment (configure **Required reviewers** for manual approval) and exclusive `concurrency`. Rollback = re-run with the previous immutable `image_tag`.
- **Trivy gate is library-only** (`TRIVY_PKG_TYPES=library`): it fails the build on HIGH/CRITICAL in **application** dependencies (actionable), while base-image **OS** CVEs are uploaded to the Security tab (SARIF, non-blocking) and bumped via Dependabot's `docker` ecosystem — so base-image churn never blocks unrelated PRs.
- **The API runtime image is slim and npm-free**: a `prod-deps` stage runs `npm ci --omit=dev` (so jest/eslint/ts-* are never shipped) and the base image's bundled global **npm/npx is removed** in the runtime stage. Because of that, `prisma` (CLI), `@prisma/adapter-pg`, and `pg` are **production** dependencies (`tsx` already was), and `docker-entrypoint.sh` puts `node_modules/.bin` on `PATH` and calls `prisma`/`tsx` directly instead of `npx`. If you touch the entrypoint or move deps, keep `prisma` + `tsx` resolvable at runtime (the boot runs migrate → backfill → seed → `node dist/main.js`). The Prisma 7 generated client is **not** in `node_modules`: it ships compiled in `dist/` and as source in `src/generated` (both copied from the build stage), so the `prod-deps` stage no longer runs `prisma generate`; `prisma.config.ts` is copied into the runtime image so `migrate deploy`/`db seed` find the datasource URL + seed command.
- Node is pinned via `apps/{api,web}/.nvmrc` (20) + `engines`; CI uses `node-version-file`. Root `.prettierrc.json` pins `singleQuote` so Prettier matches the codebase — but still don't run `prettier --write` broadly.
